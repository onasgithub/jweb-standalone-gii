 ...
 * @var jweb\gii\crud\Generator $generator
 */
		...
		return [
			'access' => [
				...
				'rules' => [
					[
						'allow' 	=> true,
						'actions'   => ['index', 'view', 'create', 'update', 'delete','autocomplete'],
						'roles'     => ['@']
					]
				]
			]
		...
		if (($model = <?= $modelClass ?>::findOne(<?= $condition ?>)) !== null) {
          ...
          $search = Yii::$app->request->get('search');
            ...
            if (!is_null($search)) {
                  ...
                  $items = $item->find()
                                  ->where(['like', 'name', $search])->limit(20)->all();
                  $data = [];
                  foreach ($items as $key => $value) {
                        $data[] = ['id' => $value->id, 'selection' => $value->name, 'text' => $value->name];
                  }
                  $out['results'] = array_values($data);
            } elseif ($id > 0) {
                           $value = $this->findModel($id);
                          $out['results'] = ['id' => $id, 'text' => $value->name];
            } else {
                          $item = new <?= $modelClass ?>;

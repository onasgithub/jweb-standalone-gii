...
class RelationProvider extends \jweb\gii\base\Provider {
            ...
            if ($relation) {
                  switch (true) {
                        case (!$relation->multiple):
                              ...
                              $url = str_replace("_id", "", $column->name);
                              $url = '/'.str_replace('_', '-', $url) . '/autocomplete';
                              $method = __METHOD__;
                              $code = <<<EOS
// generated by {$method}
                        ...
                        'ajax' =>[
                         'url' =>'{$url}',
                          ...
                          'initSelection' => new  yii\web\JsExpression('function (element, callback) {
                                                                  var id=\$(element).val();
                                                                  if (id !== "") {
                                                                        \$.ajax("{$url}?id=" + id, {dataType: "json"}).done(function(data) { console.log(data.results);callback(data.results);});
                                                                   }
}')
                        ],
...
[    
               ...
               'filterWidgetOptions' => [
                   ...
                   'data' => ArrayHelper::map({$relation->modelClass}::find()->asArray()->all(), 'id', 'name')
               ],
        'attribute'=>'{$column->name}',
        'format'=>'raw',
